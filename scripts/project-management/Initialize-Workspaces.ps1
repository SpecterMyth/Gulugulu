...
$registered = (& git -C $layout.Main worktree list --porcelain) -join "`n"
if ($registered -notmatch [regex]::Escape($layout.Release)) {
...
if ($registered -notmatch [regex]::Escape($layout.SteamOnline)) {
