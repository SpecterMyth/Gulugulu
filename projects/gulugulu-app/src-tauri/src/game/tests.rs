...
fn initial_save_has_tutorial_egg_and_bonus_coins() {
    ...
    assert_eq!(
        save.coins,
        config.initial_coins + config.historical_exp_coin_cap
    );
    assert_eq!(save.eggs.len(), 1);
    assert_eq!(save.eggs[0].slot, Some(0));
    assert_eq!(save.eggs[0].hatch_at, Some(1000 + 3));
    assert_eq!(save.onboarding.status, "active");
    assert_eq!(save.onboarding.step, "A01");
    assert_eq!(save.factory_tutorial.step, "C01");
}
...
fn hatching_records_dex_obtained_permanently() {
    ...
    assert_eq!(
        save.dex_obtained.get("guluduck"),
        Some(&2),
        "同物种累加曾获只数"
    );
...
fn collect_hatched_respects_time_and_capacity() {
    ...
    let err =
        logic_collect_hatched(&config, &mut save, "egg-full", 2000, "2026-07-07").unwrap_err();
...
fn plan_fusion_same_species_upgrades_deterministically() {
    ...
    assert_eq!(
        plan.result_species, "steamalotl",
        "同物种升阶保持物种（不掷骰）"
    );
    assert_eq!(plan.result_tier, 3, "2 阶 → 3 阶（封顶前的最后一次升阶）");
...
fn plan_fusion_cross_species_rolls_slot_ladder() {
    ...
    save.recipe_ai_slots
        .insert("fire+water".into(), vec!["aiffw1".into()]);
...
#[test]
fn fusion_caps_result_tier_and_blocks_same_species_no_gain() {
    // 经济 v2.0（EconomyRework-TrainingHall.md §2）：结果阶 = min(亲代阶+1, 3)。
    // 亲代阶已达封顶 → 同阶仍可融、照常取元素并集，**只是不涨阶**。
    let config = test_config();
    assert_eq!(config.fusion_result_tier(1), 2);
    assert_eq!(config.fusion_result_tier(2), 3);
    assert_eq!(config.fusion_result_tier(3), 3, "3 阶封顶");
    assert_eq!(config.fusion_result_tier(6), 6, "已超封顶的存量宠保持原阶");

    // 3 阶异物种融合 → 仍是 3 阶蛋，物种 = 元素并集，费用按亲代阶。
    let mut save = fresh_save(&config);
    save.onboarding.status = "completed".to_string();
    save.eggs.clear();
    ...
    let c = add_pet_at_tier(&mut save, &config, "emberfox", 3, max3);
    let d = add_pet_at_tier(&mut save, &config, "bubblefrog", 3, max3);
    logic_fuse_pets(&config, &mut save, &c, &d, 1000, "2026-07-07").unwrap();
    assert_eq!(save.coins, 500);
    assert_eq!(save.eggs[0].tier, 3, "亲代 3 阶 → 结果仍 3 阶（不涨阶）");
    assert_eq!(save.eggs[0].species, "steamalotl", "元素并集照常生效");

    // 3 阶同物种：并集即自身 + 不涨阶 = 纯亏两只 → 拦在门口。
    let mut same = fresh_save(&config);
    same.onboarding.status = "completed".to_string();
    same.eggs.clear();
    same.coins = fee3 + 500;
    let g = add_pet_at_tier(&mut same, &config, "steamalotl", 3, max3);
    let h = add_pet_at_tier(&mut same, &config, "steamalotl", 3, max3);
    let err = logic_fuse_pets(&config, &mut same, &g, &h, 1000, "2026-07-07").unwrap_err();
    assert_eq!(err, "#fusionSameSpeciesNoGain", "同物种无收益阻断：{err}");

    // 存量 6 阶宠**现在可以**融合（换物种、不涨阶）——图鉴收集的长尾路径。
    let mut top = fresh_save(&config);
    top.onboarding.status = "completed".to_string();
    top.eggs.clear();
    top.coins = config.fusion_fee_for(6) + 10;
    let max6 = config.max_level_for_tier(6);
    let e = add_pet_at_tier(&mut top, &config, "emberfox", 6, max6);
    let f = add_pet_at_tier(&mut top, &config, "bubblefrog", 6, max6);
    logic_fuse_pets(&config, &mut top, &e, &f, 1000, "2026-07-07").unwrap();
    assert_eq!(top.eggs[0].tier, 6, "6 阶合成保持 6 阶");
    assert_eq!(top.eggs[0].species, "steamalotl");
}

/// 造一座满级训练馆 + 管够的材料和金币（训练馆用例的公共起点）。
fn training_ready_save(config: &GameConfig) -> GameSave {
    let mut save = fresh_save(config);
    save.eggs.clear();
    save.coins = 10_000_000_000;
    save.training_hall_level = config.training_hall_max_level();
    for material in &config.training_materials {
        save.materials.insert(material.clone(), 999);
    }
    save
}

#[test]
fn training_upgrades_tier_in_place_and_keeps_level() {
    // 训练馆的卖点：**还是这一只**——阶数 +1，等级不清零（与融合产 1 级新个体相对）。
    let config = test_config();
    let mut save = training_ready_save(&config);
    let max1 = config.max_level_for_tier(1);
    let pet = add_pet_at_tier(&mut save, &config, "emberfox", 1, max1);
    let (material, need, coins, seconds) = config.training_step_for(1).unwrap();
    let coins_before = save.coins;
    let material_before = save.materials[&material];

    let job = logic_start_training(&config, &mut save, &pet, false, 1000, "2026-07-07").unwrap();
    assert_eq!(save.coins, coins_before - coins, "开练即扣金币");
    assert_eq!(
        save.materials[&material],
        material_before - need,
        "开练即扣材料"
    );
    assert_eq!(save.training_jobs.len(), 1);
    assert!(save.training_tutorial_boost_claimed);
    assert_eq!(
        save.training_jobs[0].done_at, 1010,
        "首次 1→2 教学只等 10 秒"
    );
    assert!(seconds > 10, "正式训练时间应明显长于教学单");
    assert_eq!(save.pets[0].tier, 1, "训练期间阶数不变");

    // 没到点收不了。
    let err = logic_collect_training(&config, &mut save, &job, 1000, "2026-07-07").unwrap_err();
    assert_eq!(err, "#trainingNotDone");

    let done = 1010;
    logic_collect_training(&config, &mut save, &job, done, "2026-07-07").unwrap();
    assert_eq!(save.pets[0].tier, 2, "收取即升一阶");
    assert_eq!(save.pets[0].level, max1, "等级保留——不是新个体");
    assert!(save.training_jobs.is_empty(), "槽位释放");
    assert_eq!(save.stats.total_tier_ups, 1);
    assert_eq!(save.stats.highest_tier, 2);
}

#[test]
fn training_gates_cover_hall_level_maxlevel_slots_and_top_tier() {
    let config = test_config();
    let max1 = config.max_level_for_tier(1);

    // 未建馆 → 拦。
    let mut unbuilt = fresh_save(&config);
    unbuilt.eggs.clear();
    unbuilt.coins = 10_000_000_000;
    let p = add_pet_at_tier(&mut unbuilt, &config, "emberfox", 1, max1);
    assert_eq!(
        logic_start_training(&config, &mut unbuilt, &p, false, 1000, "d").unwrap_err(),
        "#trainingHallLocked"
    );

    // 馆等级不够（Lv1 只能练 1→2，练 3→4 要 Lv3）。
    let mut low = training_ready_save(&config);
    low.training_hall_level = 1;
    let max3 = config.max_level_for_tier(3);
    let t3 = add_pet_at_tier(&mut low, &config, "steamalotl", 3, max3);
    let err = logic_start_training(&config, &mut low, &t3, false, 1000, "d").unwrap_err();
    assert_eq!(err, "#trainingHallTooLow|need=3", "{err}");

    let mut save = training_ready_save(&config);
    // 未满级 → 拦。
    let unripe = add_pet_at_tier(&mut save, &config, "emberfox", 1, max1 - 1);
    assert_eq!(
        logic_start_training(&config, &mut save, &unripe, false, 1000, "d").unwrap_err(),
        "#trainingNeedMaxLevel"
    );
    // 已达最高阶 → 拦（6 阶无升阶项）。
    let max6 = config.max_level_for_tier(6);
    let top = add_pet_at_tier(&mut save, &config, "prismkirin", 6, max6);
    assert_eq!(
        logic_start_training(&config, &mut save, &top, false, 1000, "d").unwrap_err(),
        "#trainingMaxTier"
    );
    // 正常开练后：同一只不能重复排队；槽位满了别人也进不来（初始 1 槽）。
    let a = add_pet_at_tier(&mut save, &config, "emberfox", 1, max1);
    let b = add_pet_at_tier(&mut save, &config, "bubblefrog", 1, max1);
    logic_start_training(&config, &mut save, &a, false, 1000, "d").unwrap();
    assert_eq!(
        logic_start_training(&config, &mut save, &a, false, 1000, "d").unwrap_err(),
        "#trainingAlreadyRunning"
    );
    assert_eq!(
        logic_start_training(&config, &mut save, &b, false, 1000, "d").unwrap_err(),
        "#trainingNoSlot"
    );
    // 扩一格槽 → b 可以进。
    logic_upgrade_training_slots(&config, &mut save, 1000, "d").unwrap();
    logic_start_training(&config, &mut save, &b, false, 1000, "d").unwrap();
    assert_eq!(save.training_jobs.len(), 2);
    assert_ne!(
        save.training_jobs[0].slot, save.training_jobs[1].slot,
        "槽位不重叠"
    );
}

#[test]
fn training_universal_material_fills_the_shortfall_only_when_allowed() {
    // 万能券（工厂 26–30 关产出）1:1 替代任意升阶材料，是后期短板材料的加速通道。
    let config = test_config();
    ...
    // 不允许用券 → 明确报缺料（带 have/need 参数供 UI 渲染）。
    let err = logic_start_training(&config, &mut save, &pet, false, 1000, "d").unwrap_err();
    assert!(err.starts_with("#trainingNoMaterial|material="), "{err}");
    ...
    assert!(
        logic_start_training(&config, &mut broke, &p2, true, 1000, "d")
            .unwrap_err()
            .starts_with("#trainingNoMaterial")
    );
}

#[test]
fn factory_claim_grants_each_level_once_per_day_and_resets_next_day() {
    // EconomyRework-TrainingHall.md §5.2：每个自然日、每关只领一次。当天冲到第 N 关 =
    // 一次性领到 1..N 的未领奖励；同日再冲只补发新突破的关卡 → 重复刷零收益。
    let config = test_config();
    let mut save = fresh_save(&config);
    save.eggs.clear();
    let iron = config.training_materials[0].clone();
    let copper = config.training_materials[1].clone();

    // 冲到第 7 关：1–5 关各 1 个铁工牌，6–7 关各 1 个铜护目镜。
    let granted = logic_claim_factory_levels(&config, &mut save, 7, 1000, "2026-07-07").unwrap();
    assert_eq!(granted.get(&iron).copied(), Some(5));
    assert_eq!(granted.get(&copper).copied(), Some(2));
    assert_eq!(save.materials[&iron], 5);
    assert_eq!(save.daily.factory_claimed_level, 7);
    assert_eq!(save.stats.factory_best_level, 7);

    // 同日重刷到第 5 关（比已领的低）→ 一无所获。
    let again = logic_claim_factory_levels(&config, &mut save, 5, 1000, "2026-07-07").unwrap();
    assert!(again.is_empty(), "重复冲不重复发");
    assert_eq!(save.materials[&iron], 5);

    // 同日突破到第 9 关 → 只补发 8、9 两关。
    let more = logic_claim_factory_levels(&config, &mut save, 9, 1000, "2026-07-07").unwrap();
    assert_eq!(more.get(&copper).copied(), Some(2), "只补新突破的关卡");
    assert_eq!(save.materials[&copper], 4);

    // 跨天 → 账本重置，同样的进度可以再领一轮。
    logic_claim_factory_levels(&config, &mut save, 5, 90_000, "2026-07-08").unwrap();
    assert_eq!(save.materials[&iron], 10, "新的一天重新发放");
    assert_eq!(save.daily.factory_claimed_level, 5);
    assert_eq!(
        save.stats.factory_best_level, 9,
        "历史最高关是高水位，不因当日回落而降"
    );

    // 超出关卡上限按上限收敛（30 关 = 万能券档）。
    let capped = logic_claim_factory_levels(&config, &mut save, 999, 90_000, "2026-07-08").unwrap();
    assert_eq!(
        capped.get(&config.universal_material).copied(),
        Some(5),
        "26–30 关产万能券"
    );
    assert_eq!(save.daily.factory_claimed_level, config.factory_max_level);
}

#[test]
fn training_hall_and_slot_upgrades_stop_at_cap() {
    let config = test_config();
    let mut save = fresh_save(&config);
    save.eggs.clear();
    save.coins = 10_000_000_000;
    assert_eq!(save.training_hall_level, 0, "训练馆要玩家自己盖");
    // 未建馆时不能先扩槽。
    assert_eq!(
        logic_upgrade_training_slots(&config, &mut save, 1000, "d").unwrap_err(),
        "#trainingHallLocked"
    );
    let mut built = 0;
    while logic_build_training_hall(&config, &mut save, 1000, "d").is_ok() {
        built += 1;
    }
    assert_eq!(built, 5, "建造 + 4 次升级 = Lv5（解锁 5→6 阶）");
    assert_eq!(save.training_hall_level, config.training_hall_max_level());

    let mut slots = 0;
    while logic_upgrade_training_slots(&config, &mut save, 1000, "d").is_ok() {
        slots += 1;
    }
    assert_eq!(slots, 3, "1 → 4 槽共 3 次扩建");
    assert_eq!(config.training_slot_count(save.training_slot_level), 4);
}
...
fn token_feed_never_spills_to_other_pets() {
    ...
    assert!(
        save.stats.first_maxlevel_done,
        "token 喂满级也应置 first_maxlevel_done"
    );
}
...
fn key_feed_only_charges_active_pet() {
    ...
    assert_eq!(
        save.pets.iter().find(|p| p.id == buddy).unwrap().stamina,
        10,
        "键盘精力绝不溢给其他宠"
    );
...
fn ledger_breakdown_diff_tracks_seeds_and_self_heals() {
    ...
    assert_eq!(
        save.last_seen_project_breakdown.get("proj"),
        Some(&bd(10, 10, 10, 10))
    );
    assert_eq!(
...
fn migrates_v2_save_to_v3() {
    ...
    assert_eq!(
        reloaded.daily.clicks, 0,
        "旧 daily 字段被忽略、新字段取默认"
    );
    ...
    assert_eq!(reloaded.version, 10, "v2 链式迁移到当前 schema");
    // v4：图鉴曾获从在册宠物播种。
    assert_eq!(
        reloaded.dex_obtained.get("guluduck"),
        Some(&1),
        "曾获账本播种"
    );
    ...
    assert_eq!(
        reloaded.last_seen_project_tokens.get("D:/proj"),
        Some(&7000)
    );
...
fn migrates_v4_backfills_recipe_ai_slots_from_custom_species() {
    ...
    assert!(
        !save.recipe_ai_slots.contains_key("fire"),
        "单元素不入 AI 槽"
    );
...
fn skin_select_rules() {
    ...
    assert_eq!(
        save.skin_selected.get("aifx1").map(String::as_str),
        Some("default")
    );
    // ws: 未导入拒绝；导入后可选。
    assert_eq!(
...
fn skin_install_dedupes_caps_and_remove_falls_back() {
    ...
    assert_eq!(
        save.species_skins.get("aifx1").unwrap().len(),
        MAX_SKINS_PER_SPECIES
    );
    assert_eq!(
...
fn skin_serde_uses_camel_case_and_roundtrips() {
    ...
    assert_eq!(
        back.skin_selected.get("aifx1").map(String::as_str),
        Some("ws:9001")
    );
}
...
fn shop_upgrade_and_tiered_egg_pricing() {
    ...
    let mut save = fresh_save(&config);
    save.eggs.clear();
    save.coins = 20_000_000; // 覆盖商店升级 50k+750k
    ...
    // 升到 Lv3 封顶后不能再升（经济 v2.0：shopMaxLevel 4→3）。
    logic_upgrade_shop(&config, &mut save, 1000, "d").unwrap(); // →3
    assert_eq!(save.shop_level, 3);
    assert!(logic_upgrade_shop(&config, &mut save, 1000, "d").is_err());
    // 商店封顶 3 阶：4 阶蛋永不可买——4~6 阶只能靠训练馆升阶得来。
    assert!(logic_buy_egg(&config, &mut save, "fire", 4, 1000, "d").is_err());
}
...
fn egg_pool_is_global_fixed_only() {
    ...
    assert_eq!(
        t1,
        vec![("emberfox".to_string(), config.egg_rarity_weight(1))]
    );
    ...
    save.custom_species.insert(
        "aiffireice".to_string(),
        sample_custom_entry(["emberfox", "frostbunny"]),
    );
    ...
    assert_eq!(
        roll_egg_species(&config, "fire", 1, 0).as_deref(),
        Some("emberfox")
    );
}
...
fn fusion_daily_mint_cap_blocks_per_recipe_and_resets() {
    ...
    .unwrap();
    assert_eq!(
        save.daily.fusion_mints.get("fire+water").copied(),
        Some(1),
        "跨天计数应重置"
    );
}
...
fn collect_tier_follows_egg_not_species() {
    ...
    let pet_id = apply_collect(
        &config,
        &mut save,
        0,
        2000,
        Some(("emberfox".into(), "item-x".into(), 102)),
    );
...
fn egg_daily_mint_cap_blocks_and_resets_next_day() {
    // test_config() 载入生产 config.json：eggDailyMintCaps = [10,8,6,3] → 3 阶上限 6。
    // 商店封顶 Lv3（经济 v2.0）后 4 阶蛋不可买，故上限验证改用 3 阶这一最高可售档。
    let config = test_config();
    let mut save = fresh_save(&config);
    save.coins = 500_000_000; // 覆盖 7×一般 3 阶蛋（115.6 万/颗）+ 1×2 阶蛋（v1.3 乘数 85）
    save.shop_level = 3; // 可买 3 阶蛋（封顶）
                         // 头 6 次成功，第 7 次拒绝。
    for _ in 0..6 {
        logic_buy_egg(&config, &mut save, "normal", 3, 1000, "2026-07-07").unwrap();
    }
    let err = logic_buy_egg(&config, &mut save, "normal", 3, 1000, "2026-07-07").unwrap_err();
    assert!(
        err.starts_with("#eggDailyCap|recipe=normal|tier=3|cap="),
        "达上限应拒绝（协议键 + 属性/阶参数）: {err}"
    );
    assert_eq!(save.daily.egg_mints.get("normal:3").copied(), Some(6));
    // 不同阶独立计数：normal 2 阶仍可买（上限 8）。
    logic_buy_egg(&config, &mut save, "normal", 2, 1000, "2026-07-07").unwrap();
    assert_eq!(save.daily.egg_mints.get("normal:2").copied(), Some(1));
    // 跨天重置：换一天后 normal 3 阶又能买、计数归 1。
    logic_buy_egg(&config, &mut save, "normal", 3, 1000, "2026-07-08").unwrap();
    assert_eq!(
        save.daily.egg_mints.get("normal:3").copied(),
        Some(1),
        "跨天计数应重置"
    );
}
...
fn logic_register_workshop_species_registers_appearance_slot_and_skin() {
    ...
    assert!(logic_register_workshop_species(
        &config,
        &mut save,
        codename,
        entry.clone(),
    ...
    assert_eq!(
        save.workshop_published.get(codename).map(String::as_str),
        Some("")
    );
    // 幂等：已注册再调 → Ok 且不重复入库。
    assert!(logic_register_workshop_species(&config, &mut save, codename, entry, skin).is_ok());
    assert_eq!(
        save.species_skins.get(codename).map(Vec::len),
        Some(1),
        "幂等不重复入库"
    );
}
...
fn custom_species_v2_fields_are_backward_compatible_and_roundtrip() {
    ...
    let visual = legacy_value
        ...
        .unwrap();
    visual.remove("motionPreset");
    visual.remove("reactionProfile");
    let legacy: CustomSpeciesEntry = serde_json::from_value(legacy_value).unwrap();
    assert!(legacy.design_meta.is_none());
    assert!(legacy.visual.motion_preset.is_none());
    assert!(legacy.visual.reaction_profile.is_none());

    let mut v2 = legacy;
    v2.visual.motion_preset = Some("bound".to_string());
    v2.visual.reaction_profile = Some("mischievous".to_string());
    v2.design_meta = Some(GeneratedDesignMeta {
        prompt_version: "pet-gen-v2".to_string(),
        prototype: "水獭".to_string(),
        archetype: "四足兽".to_string(),
        hero_feature: "桨形大尾".to_string(),
        personality: "淘气".to_string(),
        palette_family: "湖蓝暖橙".to_string(),
        quality_score: 88.5,
    });
    let restored: CustomSpeciesEntry =
        serde_json::from_str(&serde_json::to_string(&v2).unwrap()).unwrap();
    assert_eq!(restored.visual.motion_preset.as_deref(), Some("bound"));
    assert_eq!(
        restored.visual.reaction_profile.as_deref(),
        Some("mischievous")
    );
    assert_eq!(
        restored
            .design_meta
            .as_ref()
            .map(|meta| meta.prompt_version.as_str()),
        Some("pet-gen-v2")
    );
}
...
fn ai_fusion_start_consumes_and_creates_pending_egg() {
    ...
    assert_eq!(
        pending.parents,
        ["emberfox".to_string(), "frostpeng".to_string()]
    );
...
fn ai_fusion_resolve_rejects_taken_codename_and_missing_egg() {
    ...
    // 与静态目录撞名。
    let err = logic_resolve_fusion_egg(
        &config,
        &mut save,
        &egg_id,
        "guluduck",
        sample_custom_entry(["a", "b"]),
    )
    .unwrap_err();
    assert!(
        err.starts_with("#codenameTaken|codename="),
        "撞名应拒绝: {err}"
    );
...
#[test]
fn onboarding_rewards_are_exact_idempotent_and_capacity_exempt() {
    let config = test_config();
    let mut save = fresh_save(&config);
    save.eggs.clear();
...
fn onboarding_first_fire_pet_stays_level_one_and_does_not_replace_companion() {
    ...
    let egg_id = logic_buy_egg(&config, &mut save, "fire", 1, 2_000, "2026-07-07").unwrap();
    let egg = save.eggs.iter().find(|egg| egg.id == egg_id).unwrap();
    assert_eq!(egg.slot, None);
    assert_eq!(egg.hatch_at, None);
    assert!(save.tutorial_first_egg_bought);
    assert_eq!(save.coins, 137, "教学火蛋应全额报销，不能吞掉现有金币");

    logic_place_egg(&config, &mut save, &egg_id, 1, 2_001, "2026-07-07").unwrap();
    let egg = save.eggs.iter().find(|egg| egg.id == egg_id).unwrap();
    assert_eq!(egg.slot, Some(1));
    assert_eq!(egg.hatch_at, Some(2_009));

    let normal_id = add_pet(&mut save, &config, "guluduck", 1);
    save.active_pet_id = Some(normal_id.clone());
    save.onboarding.step = "A15".to_string();
    let fire_id = logic_collect_hatched(&config, &mut save, &egg_id, 2_009, "2026-07-07").unwrap();
    assert_eq!(save.active_pet_id, Some(normal_id));
    let fire = save.pets.iter().find(|pet| pet.id == fire_id).unwrap();
    assert_eq!(fire.level, 1);

    save.onboarding.tutorial_work_clicks = 20;
    logic_advance_onboarding(&config, &mut save, "A15", 2_009).unwrap();
    assert_eq!(save.onboarding.step, "A16");
    assert_eq!(save.onboarding.tutorial_work_clicks, 0);
}
...
fn onboarding_facility_upgrades_are_fully_reimbursed() {
    ...
    logic_upgrade_hatchery(&config, &mut save, 1_000, "2026-07-07").unwrap();
...
#[test]
fn onboarding_rejects_future_receipts_and_factory_cursor_cannot_skip() {
    let config = test_config();
    let mut save = fresh_save(&config);
    let err = logic_advance_onboarding(&config, &mut save, "A03", 1000).unwrap_err();
    assert!(err.contains("#onboardingOutOfOrder"));
    assert_eq!(save.onboarding.step, "A01");

    logic_set_factory_tutorial_step(&mut save, "C01").unwrap();
    assert_eq!(save.factory_tutorial.step, "C02");
    let err = logic_set_factory_tutorial_step(&mut save, "C03").unwrap_err();
    assert!(err.contains("#factoryTutorialOutOfOrder"));
    assert_eq!(save.factory_tutorial.step, "C02");

    logic_set_factory_tutorial_step(&mut save, "C12").unwrap();
    assert_eq!(save.factory_tutorial.status, "completed");
    assert_eq!(save.factory_tutorial.step, "C12");
    logic_set_factory_tutorial_step(&mut save, "C12").unwrap();
}

#[test]
fn capacity_exempt_roster_does_not_block_a_normal_hatch() {
    let config = test_config();
    let mut save = fresh_save(&config);
    save.eggs.clear();
    add_pet(&mut save, &config, "guluduck", 1);
    add_pet(&mut save, &config, "emberfox", 1);
    assert_eq!(occupied_pet_count(&save), 2);

    save.onboarding.step = "C12".to_string();
    logic_advance_onboarding(&config, &mut save, "C12", 2000).unwrap();
    assert_eq!(occupied_pet_count(&save), 2);

    save.eggs.push(EggInstance {
        id: "capacity-check-egg".into(),
        species: "guluduck".into(),
        tier: 1,
        hatch_kind: "normal".into(),
        slot: Some(0),
    ...
    logic_collect_hatched(&config, &mut save, "capacity-check-egg", 2000, "2026-07-07").unwrap();
    assert_eq!(
        occupied_pet_count(&save),
        config.yard_capacity_for(save.yard_level) as usize
    );
}

#[test]
fn v9_migration_retires_veterans_but_keeps_true_newcomers_active() {
    let config = test_config();
    let mut veteran = fresh_save(&config);
    veteran.version = 9;
    veteran.tutorial_first_fusion_done = true;
    assert!(migrate_save(
        &config,
        &mut veteran,
        &BTreeMap::new(),
    ...
    assert_eq!(veteran.onboarding.status, "completed");
    assert_eq!(veteran.onboarding.step, "DONE");

    let mut newcomer = fresh_save(&config);
    newcomer.version = 9;
    newcomer.onboarding.status = "completed".to_string();
    newcomer.onboarding.step = "DONE".to_string();
    assert!(migrate_save(
        &config,
        &mut newcomer,
        &BTreeMap::new(),
    ...
    assert_eq!(newcomer.onboarding.status, "active");
    assert_eq!(newcomer.onboarding.step, "A01");
}

#[test]
fn first_training_build_can_reimburse_and_first_job_is_ten_seconds() {
    let config = test_config();
    let mut save = fresh_save(&config);
    save.eggs.clear();
    save.coins = 0;
    let max1 = config.max_level_for_tier(1);
    let pet = add_pet_at_tier(&mut save, &config, "emberfox", 1, max1);
    let (material, need, fee, normal_seconds) = config.training_step_for(1).unwrap();
    save.materials.insert(material, need);

    logic_build_training_hall(&config, &mut save, 1000, "2026-07-07").unwrap();
    assert_eq!(save.training_hall_level, 1);
    assert_eq!(save.coins, 0, "建馆教学报销不可变成可挪用金币");
    ...
    let training = save
        ...
        .unwrap();
    assert_eq!(training.done_at, 1010);
    assert!(normal_seconds > 10);
    assert!(save.training_tutorial_boost_claimed);
}

#[test]
fn stamina_rescue_wakes_once_and_is_idempotent() {
    let config = test_config();
    let mut save = fresh_save(&config);
    save.eggs.clear();
    let pet_id = add_pet(&mut save, &config, "guluduck", 1);
    ...
    logic_claim_stamina_tutorial_rescue(&config, &mut save).unwrap();
    let pet = save.pets.iter().find(|pet| pet.id == pet_id).unwrap();
    assert_eq!(pet.stamina, config.wake_threshold);
    assert!(!pet.exhausted);
    assert!(save.stamina_tutorial_rescue_claimed);
...
#[test]
fn complete_onboarding_receipt_simulation_has_no_dead_end_or_double_grant() {
    let config = test_config();
    let mut save = fresh_save(&config);
    let route = [
    ...
    for step in route {
        if step.starts_with('C') {
            logic_set_factory_tutorial_step(&mut save, step).unwrap();
        }
        logic_advance_onboarding(&config, &mut save, step, 1000).unwrap();
        let pets_after_first_receipt = save.pets.len();
        logic_advance_onboarding(&config, &mut save, step, 1000).unwrap();
        assert_eq!(
            save.pets.len(),
            pets_after_first_receipt,
            "double click/duplicate receipt at {step} must be idempotent"
        );
    }
    assert_eq!(save.onboarding.status, "completed");
    assert_eq!(save.onboarding.step, "DONE");
    assert_eq!(save.factory_tutorial.status, "completed");
    assert!(save.onboarding.starter_trio_claimed);
    assert!(save.onboarding.post_practice_roster_claimed);
    assert_eq!(save.capacity_exempt_pet_ids.len(), 6);
