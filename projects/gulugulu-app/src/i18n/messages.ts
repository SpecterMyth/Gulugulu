...
export const MESSAGES: Record<Language, Record<string, string>> = migrateLegacyLanguageMap({
  zh: {
    ...
    trainingHallLocked: "还没有训练馆——先在孵化区左边把它盖起来",
    trainingHallMaxLevel: "训练馆已是最高等级",
    trainingHallTooLow: "训练馆等级不够，升到 Lv{need} 才能做这一级升阶",
    trainingSlotMaxLevel: "训练位已扩到上限",
    trainingNeedMaxLevel: "要先把它点到满级才能升阶",
    trainingMaxTier: "已达最高阶，无法再升",
    trainingNoMaterial: "{material}不足（{have}/{need}）——去工厂打工赚材料",
    trainingNoSlot: "训练位都占满了，等一只出师或扩建训练位",
    trainingAlreadyRunning: "这只已经在训练中了",
    trainingJobNotFound: "找不到这次训练",
    trainingNotDone: "训练还没结束",
    ...
    fusionSameSpeciesNoGain: "同物种在此阶融合不会升阶，也不会换物种——去训练馆升阶吧",
    ...
    fusionIdeating: "正在比较 3 个宠物概念",
    fusionDrawing: "正在绘制胜出概念的三视图",
    fusionReviewing: "正在检查剪影、脸部和姿态",
    fusionRevising: "正在按质量报告定向修稿",
  ...
  en: {
    ...
    trainingHallLocked: "No Training Hall yet — build it left of the hatchery first",
    trainingHallMaxLevel: "The Training Hall is already at max level",
    trainingHallTooLow: "Training Hall too low — reach Lv{need} for this tier-up",
    trainingSlotMaxLevel: "All training slots are already built",
    trainingNeedMaxLevel: "Get it to max level before tiering up",
    trainingMaxTier: "Already at the top tier",
    trainingNoMaterial: "Not enough {material} ({have}/{need}) — earn more on the factory floor",
    trainingNoSlot: "Every training slot is busy — graduate one or add a slot",
    trainingAlreadyRunning: "This buddy is already in training",
    trainingJobNotFound: "Couldn't find that training session",
    trainingNotDone: "Training isn't finished yet",
    ...
    fusionSameSpeciesNoGain:
      "Fusing the same species at this tier gains neither a tier nor a new species — use the Training Hall instead",
    ...
    providerError: "{provider}: {err}",
    fusionIdeating: "Comparing three pet concepts",
    fusionDrawing: "Drawing the winning concept in three views",
    fusionReviewing: "Reviewing silhouette, face, and poses",
    fusionRevising: "Revising the design from its quality report",
