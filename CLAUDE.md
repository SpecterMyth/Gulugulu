...
- `fusion_gen.rs` — AI species-fusion worker. Spawns the local Codex/Claude CLI (via `cli_spawn`) off the UI thread to design new species, validates the returned JSON against a strict schema, and pushes progress on `fusion://progress`. A background worker drains pending-fusion eggs one at a time; on generation failure it **auto-retries in-session** up to `MAX_FUSION_ATTEMPTS` (and startup recovery resets `failed`/`generating` → `pending`), before the egg falls back to hatching the recipe's canonical slot-0 species at its deadline. The pick order is pure and unit-tested (`pick_fusion_job`): new `pending` eggs first, then retryable `failed` ones.
