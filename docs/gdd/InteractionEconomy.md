...
| 成就口径 | `total_tokens_fed` 仍按加权喂养单位累计；「代码盛宴」改读 `total_tokens_observed ≥ 1B`，四类原始 Token 等权求和、不区分是否产出；夜猫子旗标两条喂养路径都保留；`first_maxlevel_done` 点击与 Token 喂养两路都置位（Token 喂满级也能拿「首次满级」） |
